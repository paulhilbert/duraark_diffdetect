...
enum class rep_type_t : int { ifc, pc };


int main (int argc, char const* argv[]) {
    std::string            file_in;
    std::string            file_assoc;
    std::string            file_out;
    Eigen::Vector3f new_col;

    po::options_description desc("diffdetect command line options");
    desc.add_options()
        ("help,h",   "Help message")
        ("input,i",  po::value<std::string>(&file_in) ->required(), "Input E57N File")
        ("assoc",  po::value<std::string>(&file_assoc) ->default_value(""), "Association RDF file")
        ("output,o",  po::value<std::string>(&file_out) ->required(), "Output E57N File")
        ("red,r",  po::value<float>(&new_col[0]) ->default_value(0.f), "New points color - red")
        ("green,g",  po::value<float>(&new_col[1]) ->default_value(1.f), "New points color - green")
        ("blue,b",  po::value<float>(&new_col[2]) ->default_value(0.4f), "New points color - blue")
    ;
    ...
    if (!fs::exists(path_in)) { std::cerr << "Input file \"" << file_in << "\" does not exist!" << "\n"; return 1; }

    std::string guid;
    std::vector<Eigen::Vector3f> colors;
    e57_pcl::cloud_normal_t::Ptr cloud = e57_pcl::load_e57_cloud_with_normals(path_in.string(), guid, false, nullptr, &colors);

    if (file_assoc != "") {
        fs::path path_assoc(file_assoc);
        if (!fs::exists(path_assoc)) {
            std::cerr << "Association file \"" << file_assoc << "\" does not exist!" << "\n";
            return 1;
        }
        duraark_rdf::turtle_input input(file_assoc);
        std::map<std::string, uint32_t> possible_hits_tmp;
        auto assocs = parse_subset_associations(input, possible_hits_tmp, nullptr);
